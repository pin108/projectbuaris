...
class GalangdanakegiatanbantuanorangsakitController extends Controller
{
    //
    ...
    public function index()
    {
        ...
        $kategoriRecords = Kategorigalangdana::where('jenis_kategori', 'panti asuhan')->get();
        if ($user->ktp == null) {
            $message = "Mohon Lengkapi Data Anda Pada Menu Profile";
            return $this->notif($message);
            // return view('pages.message.index', compact('message'));
        } else {
            return view('pages.galangdana.registergalangdanaorangsakit', compact('userId', 'kategoriRecords'));
        }
    }
