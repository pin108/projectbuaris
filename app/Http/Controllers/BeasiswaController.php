...
class BeasiswaController extends Controller
{
    ...
    public function store(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            // Check if the record already exists in the database based on certain criteria (e.g., email)
            $existingRecord = pendaftaran_beasiswa::where('id_peserta', $request->input('id_peserta'))->first();
            if ($existingRecord) {
                // Handle duplicate entry scenario with an error message
                return redirect()->back()->with('error', 'Anda sudah mendaftar, tidak bisa mendaftar beasiswa lebih dari satu.');
            }
